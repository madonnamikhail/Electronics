...
class SinglePageController extends Controller
{
    public function getProoductSinglePage($id)
    {
        $product=Product::Join('offer_product','offer_product.product_id','=','products.id','left outer')
                    ...
                    'products.details_en as product_details_en','products.details_ar as product_details_ar','offers.*','ratings.*','users.name as user_name',DB::raw('products.price *((100-offers.discount)/100) AS price_after_discount')
                    /*,DB::raw('count(*) as user_rating_count')*/,
                    DB::raw("COUNT(ratings.user_id) as count_click"))
                    ->groupBy('ratings.product_id','products_id','products.name_en','products.name_ar','product_photo','products.price'/*
                    'product_details_en','product_details_ar','offers.*','ratings.*','user_name'*/)
